...
$SYMANTECVERIFICATION = Invoke-Command -Session $SESSION -ScriptBlock{
	$Answer = Get-WmiObject Win32_Product -Filter "Name='Symantec Endpoint Protection'" | Select *
	If ($Answer.Name)
	{
...
Write-Host "Install was: " $SYMANTECVERIFICATION
IF($SYMANTECVERIFICATION -eq "Success")
{
